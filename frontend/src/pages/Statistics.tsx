...
import { PieChart, Pie, ResponsiveContainer, Legend } from 'recharts'; // Cell удален
import { statisticsApi, type CategoryStat } from '../api/statistics';

const { Title, Text } = Typography;

// Цвета для диаграммы
const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#AF19FF', '#FF4560', '#FF6384', '#36A2EB'];
...
const Statistics: React.FC = () => {
    ...
    // Подготовка данных для Pie Chart с вычислением процентов и ЦВЕТОВ
    const { totalAmount, chartData } = useMemo(() => {
        ...
        const chart = data.map((item, index) => ({
            ...
            fill: COLORS[index % COLORS.length], // Присваиваем цвет прямо в данные
        }));
    ...
    return (
        <div style={{ padding: '24px' }}>
            <Flex justify="space-between" align="center" style={{ marginBottom: 24 }}>
                <Title level={2} style={{ margin: 0 }}>Статистика расходов</Title>
                <Select
                    value={period}
            ...
            {loading ? (
                ...
                <Flex gap="large" wrap="wrap">
                    ...
                    <Card title="Распределение по категориям" style={{ flex: 1, minWidth: 300 }}>
                            ...
                            <ResponsiveContainer>
                                <PieChart>
                                    <Pie
                                        ...
                                        cy="50%"
                                        outerRadius={120}
                                        // fill="#8884d8" <- Убираем фиксированный цвет, так как он теперь в данных
                                        label={({ name, percent }) => {
                                    ...
                                    >
                                        {/* Cell больше не нужен! Цвета берутся из поля fill в chartData */}
                                    </Pie>
                    ...
                    >
                        <Table
                            dataSource={data}
                            columns={columns}
                            rowKey={(record) => record.categoryId?.toString() || record.categoryName}
                            pagination={false}
                            bordered
                            size="middle"
                        />
